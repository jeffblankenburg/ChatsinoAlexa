...
test("Has active game", async () => {
  ...
  const checkOpeningHandResult = [{fields:{
  ...
  cashier.isValidWager.mockReturnValue(checkWagerResult);
  data.getGamesByUserRecordId.mockResolvedValue(checkOpeningHandResult);
  const result = await poker.play(user, 10);
  expect(result.status).toBe("ACTIVE_GAME");
...
test("No active game", async () => {
  const user = userFactory.createUser(100);
  const checkWagerResult = {isValid: true, status: "VALID_WAGER", minimum: 0, maximum: 50};
  const checkOpeningHandResult = [];
  const createGameResult = {GameTypeId: [helper.VIDEOPOKER], Status: "Active",UserId: [user.fields.RecordId],Point: 0}
  cashier.isValidWager.mockReturnValue(checkWagerResult);
  data.getGamesByUserRecordId.mockResolvedValue(checkOpeningHandResult);
  data.createGame.mockResolvedValue(createGameResult);
  const result = await poker.play(user, 10);
  expect(result.status).toBe("BEFORE_DRAW");
});
